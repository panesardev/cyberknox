...
@Component({
  ...
  template: `
    ...
    <div class="{{ modal.isOpen() ? 'modal-open' : 'modal-close' }} fixed z-[101] inset-0 px-3 md:px-10 py-10 select-none">
      <div class="bg-white rounded p-6 md:p-8 mx-auto {{ width() ? width() : 'max-w-md' }}">
        <div class="flex justify-between items-center border-b-2 border-slate-100 gap-6 pb-2 mb-4">
          <h1 class="font-semibold text-lg">{{ heading() }}</h1>
          <a class="float-right text-red-500 hover:underline cursor-pointer" (click)="modal.close()">Close</a>
        </div>
        <ng-content/>
      </div>
    </div>
